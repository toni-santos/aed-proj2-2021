...
template <class K, class V> void MinHeap<K, V>::upHeap(int i) {
    while (i > 1 &&
...
template <class K, class V> void MinHeap<K, V>::downHeap(int i) {
...
template <class K, class V> void MinHeap<K, V>::swap(int i1, int i2) {
    Node tmp = a[i1];
    a[i1] = a[i2];
    a[i2] = tmp;
    pos[a[i1].key] = i1;
...
MinHeap<K, V>::MinHeap(int n, const K &notFound)
    : KEY_NOT_FOUND(notFound), size(0), maxSize(n), a(n + 1) {}

// Return number of elements in the heap
template <class K, class V> int MinHeap<K, V>::getSize() { return size; }

// Heap has key?
template <class K, class V> bool MinHeap<K, V>::hasKey(const K &key) {
...
template <class K, class V> K MinHeap<K, V>::removeMin() {
    if (size == 0)
